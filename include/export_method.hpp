...
void wrapper(uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
  T(r0, r1, r2, r3);
  shizu::obj_api::svc(shizu::obj_api::svc_num::EXIT_METHOD, 0, 0, 0, 0);
}
...
void export_method(uint32_t num = 0) {
  auto result =
      shizu::obj_api::svc(shizu::obj_api::svc_num::GET_CURRENT_OBJ_ID, 0, 0, 0);
  shizu::obj_api::svc(shizu::obj_api::svc_num::EXPORT_METHOD, num,
                      reinterpret_cast<uint32_t>(wrapper<T>), 0);
}
